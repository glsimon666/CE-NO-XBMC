...
bool CAMLCodec::OpenDecoder(CDVDStreamInfo &hints, enum ELType dovi_el_type)
{
  ...
  bool player_led(CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt(CSettings::SETTING_COREELEC_AMLOGIC_DV_LED) == AML_DV_PLAYER_LED);
  bool dv_enable(device_support_dv && !user_dv_disable &&
    hints.hdrType == StreamHdrType::HDR_TYPE_DOLBYVISION && (player_led || aml_display_support_dv() || hints.dovi.dv_profile == 5));
